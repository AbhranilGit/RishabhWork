...
def main():
    ...
    with st.sidebar:
        ...
        # Language dropdown
        languages = course_df['language'].unique()
        language = st.selectbox("Select Language", languages)
        
        # Course Code dropdown (filtered based on language)
        course_codes = course_df[course_df['language'] == language]['course_code'].unique()
        course_code = st.selectbox("Select Course Code", course_codes)
    ...
    if generate_button and blog_topic and subtitle:
        # Progress bar
        ...
        try:
            ...
            title = beautify(blog_topic,language)
            status_text.text("Processing course data...")
            ...
            status_text.text("Generating blog structure...")
            ...
            status_text.text("Producing final output...")
            ...
            # Display Title
            st.subheader("Title")
            st.code(details["title"], language=None)

            # Display Meta Contents
            st.subheader("Meta Contents")
            st.code(details["meta_contents"], language=None)

            # Display HTML Snippet
            st.subheader("HTML Snippet")
            with st.expander("Show HTML Snippet", expanded=True):
                st.code(details["html_snippet"], language='html')

            # Download images
            st.subheader("Download Images")
            image_directory = f'Blogs/Organization/images/{blog_identifier}/'
            if os.path.exists(image_directory):
                zip_file = create_zip_file(image_directory)
                st.download_button(
